...
`,[userId]);
        ...
        return NextResponse .json({message:"GET /api/certificates: successful",meta:courseDetails.rows,user_meta:myCourses.rows,status:200})
    }
    catch(err){
        console.error(err);
        return NextResponse.json({message:"server failed",meta:null,user_meta:null, status:502});
    }
}
